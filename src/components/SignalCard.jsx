...
export default function SignalCard({ sig, isNew }) {
  ...
  const [stratOpen, setStratOpen] = useState(false); // strategy section toggle
  ...
  const outcomeCol = sig.outcome === 'win' ? T.accent
                   : sig.outcome === 'partial' ? T.yellow
                   : sig.outcome === 'loss' ? T.red : T.muted;
  ...
  // Build the zone-type label from signal fields
  const zoneLabel = sig.isBreaker && sig.hasFVG ? 'Breaker+FVG'
                  : sig.isBreaker ? 'Breaker'
                  : sig.hasOB && sig.hasFVG ? 'OB+FVG'
                  : sig.hasOB ? 'OB'
                  : sig.hasFVG ? 'FVG'
                  : null;
  const zoneColor = sig.isBreaker ? T.violet
                  : (sig.hasOB && sig.hasFVG) ? T.teal
                  : sig.hasOB ? T.accent
                  : T.blue;
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4, flexWrap: 'wrap' }}>
            ...
            {zoneLabel && <Pill label={zoneLabel} color={zoneColor} sz={8} />}
            {isNew && <Pill label="NEW" color={T.accent} sz={8} />}
            {isClosed && sig.outcome && <Pill label={sig.outcome.toUpperCase()} color={outcomeCol} sz={8} />}
      ...
      {expanded && (
        <div
          ...
          style={{ marginTop: 10, borderTop: `1px solid ${T.border}`, paddingTop: 10 }}
        >
          {/* SL / TP grid */}
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 8, marginBottom: 10 }}>
            {[
              ...
              ['TP2', sig.tp2?.toFixed(dec(sig.pair)), T.blue],
            ].map(([l, v, c]) => (
          ...
          <div style={{ marginBottom: 10 }}>
            {/* Toggle header */}
            <button
              onClick={() => setStratOpen(s => !s)}
              style={{
                width: '100%', display: 'flex', justifyContent: 'space-between',
                alignItems: 'center', padding: '7px 10px', borderRadius: 6,
                background: stratOpen ? fa(T.blue) : T.dim,
                border: `1px solid ${stratOpen ? T.blue + '44' : T.border}`,
                color: stratOpen ? T.blue : T.muted,
                fontFamily: 'monospace', fontSize: 9, cursor: 'pointer',
                fontWeight: 700, marginBottom: stratOpen ? 0 : undefined,
              }}
            >
              <span>📋 STRATEGY · WHY THIS TRADE</span>
              <span>{stratOpen ? '▲' : '▼'}</span>
            </button>
              ...
              <div style={{
                ...
                {zoneLabel && (
                  <div style={{ marginBottom: 8 }}>
                    <span style={{ fontSize: 7, color: T.muted, letterSpacing: '0.08em' }}>ZONE TYPE </span>
                    <span style={{
                      fontSize: 9, fontWeight: 700, color: zoneColor,
                      padding: '1px 7px', borderRadius: 3,
                      background: fa(zoneColor), border: `1px solid ${md(zoneColor)}`,
                    }}>{zoneLabel}</span>
                  </div>
                )}
                ...
                <div style={{
                  ...
                  gap: '4px 16px', marginBottom: 10, fontSize: 8,
                }}>
                  {[
                    ['CHoCH',        sig.choch,        T.violet],
                    ['Liq sweep',    sig.sweep,         T.pink],
                    ['IDM',          sig.idmConfirmed,  T.pink],
                    ['IDM missing',  sig.hasOB && !sig.idmConfirmed, T.red],
                    ['OTE',          sig.inOTE,         T.violet],
                    ['Candle close', sig.candleClose,   T.teal],
                    ['Inst. candle', sig.strongMom,     T.amber],
                    ['Low volume',   sig.volumeQuiet,   T.orange],
                  ].filter(([, val]) => val).map(([label, , color]) => (
                ...
                {/* OTE detail */}
                {sig.inOTE && sig.oteFibPct > 0 && (
                  <div style={{ fontSize: 8, color: T.violet, marginBottom: 6 }}>
                    OTE depth: {((sig.oteFibPct || 0) * 100).toFixed(0)}% of the swing
                    (62–79% fib retracement zone)
                  </div>
                )}
                ...
                <div style={{
                  display: 'grid', gridTemplateColumns: '1fr 1fr',
                  gap: '3px 16px', fontSize: 8, marginBottom: 10,
                }}>
                  {[
                    ['Session',    sig.session,       T.lime],
                    ['Regime',     sig.regime,        regColor],
                    ['LTF bias',   sig.ltfDir,        sig.ltfDir === 'bull' ? T.accent : sig.ltfDir === 'bear' ? T.red : T.muted],
                    ['HTF bias',   sig.htfDir,        sig.htfDir === 'bull' ? T.accent : sig.htfDir === 'bear' ? T.red : T.muted],
                    ['P/D zone',   sig.pdZone,        sig.pdZone === 'discount' ? T.accent : sig.pdZone === 'premium' ? T.red : T.muted],
                    ['Zone age',   sig.zoneAgeLabel,  T.teal],
                    ['SL method',  sig.slMethod,      T.blue],
                    ['ATR',        sig.curATR > 0 ? sig.curATR.toFixed(5) : null, T.muted],
                ...
                {sig.breakdown && (
                  <>
                    <div style={{ fontSize: 7, color: T.muted, letterSpacing: '0.08em', marginBottom: 5 }}>
                      SCORE BREAKDOWN
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '2px 8px' }}>
                      {Object.entries(sig.breakdown).map(([k, v]) => {
                        ...
                        const col2 = isPos ? (BD_COLORS[k] || T.accent) : T.red;
                        return (
                          <div key={k} style={{
                            display: 'flex', justifyContent: 'space-between', fontSize: 8,
                            background: isPos ? fa(col2) : isNeg ? fa(T.red) : 'transparent',
                            borderRadius: 3, padding: '2px 5px',
                          }}>
                            <span style={{ color: T.muted }}>{BD_LABELS[k] || k}</span>
                            <span style={{ color: isPos ? col2 : isNeg ? T.red : T.muted, fontWeight: 700 }}>
                              {v > 0 ? `+${v}` : v}
                            </span>
                          </div>
                        );
          ...
          <div style={{ background: fa(regColor), border: `1px solid ${md(regColor)}`, borderRadius: 7, padding: '6px 10px', fontSize: 8, marginBottom: 8 }}>
            ...
            <span style={{ color: T.muted }}>
              {sig.regime === 'ranging'  ? '— Score −0.8 · Kelly halved · TP2 = 3R'
              : sig.regime === 'volatile'? '— Score −0.5 · Elevated risk'
              :                            '— Trending · Full score applied · TP2 = 2.5R'}
            </span>
          </div>

          {/* HTF liq */}
          {sig.htfLiqLabel && !stratOpen && (
            <div style={{ fontSize: 8, color: T.blue, marginBottom: 8 }}>
              TP2 target: {sig.htfLiqLabel}
            </div>
          )}
